...
#pragma unused(decisionHandler)
    if([ADNTLMHandler isChallengeCancelled]){
        _complete = YES;
        dispatch_async( dispatch_get_main_queue(), ^{[self.delegate webAuthenticationDidCancel];});
    ...
    if ([[[navigationAction.request.URL scheme] lowercaseString] isEqualToString:@"browser"]) {
        ...
        [[UIApplication sharedApplication] openURL:[[NSURL alloc] initWithString:requestURL] options:@{} completionHandler:nil];
        decisionHandler(WKNavigationActionPolicyCancel);
        return;
    }
...
#pragma unused(error)
    ...
    if ([error.domain isEqualToString:NSURLErrorDomain] && (error.code == NSURLErrorUnsupportedURL || error.code == NSURLErrorCannotFindHost))
    {
        ...
        {
            _complete = YES;
            dispatch_async( dispatch_get_main_queue(), ^{ [self.delegate webAuthenticationDidCompleteWithURL:url]; } );
            return;
        }
    }
    ...
    if (_delegate)
    {
        ...
        if([ADNTLMHandler isChallengeCancelled]){
            ...
            dispatch_async( dispatch_get_main_queue(), ^{ [self.delegate webAuthenticationDidFailWithError:error]; } );
        }
    }
    else
    {
...
- (void) failWithTimeout{
    ...
    [self webView: _webView didFailNavigation: nil withError: [NSError errorWithDomain: NSURLErrorDomain code: NSURLErrorTimedOut userInfo:nil]];
}
